"""Logging setup for the docparser CLI.
...
# so every CLI module logging under ``docling_pipeline.cli.*`` is captured.
CLI_LOGGER_NAME = "docparser"

_stderr_handler: logging.Handler | None = None


def configure_logging(level: int = logging.INFO, quiet: bool = False) -> None:
    """Configure the root ``docparser`` logger to emit to stderr.
