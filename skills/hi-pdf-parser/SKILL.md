---
name: hi-pdf-parser
description: Parse PDF files into Markdown plus images/manifest fully offline via the hi-pdf-parser CLI (pymupdf-only, no docling/OCR/VLM). Invoke when the user needs to extract or convert a PDF's text/content to Markdown in an offline environment.
---

# hi-pdf-parser — Offline PDF Parsing CLI

Parse PDF files into clean Markdown plus extracted images and a structured manifest, using the installed `hi-pdf-parser` CLI. This is a lightweight, fully offline tool.
...
This skill assumes the `hi-pdf-parser` command is already installed and on `PATH`
(it is the console-script entry point bundled in the `hi-pdf-parser` Python package).
...
### If `hi-pdf-parser: command not found`

The package is not installed in the active environment. Use the bundled
`hi-pdf-parser` package from Git. This requires `uv` and network access to
`github.com`.
...
```bash
hi-pdf-parser [-v|-vv|--quiet] parse <file.pdf> [options]
hi-pdf-parser [-v|-vv|--quiet] batch <file.pdf>... [options]
hi-pdf-parser batch --from-file <list.txt> [options]
```
...
hi-pdf-parser parse report.pdf

# Specific page range, verbose
hi-pdf-parser -v parse report.pdf --pages 2-4 --out ./out

# Batch, keep going, NDJSON to stdout
hi-pdf-parser batch a.pdf b.pdf --out ./out

# Batch from a manifest file, stop at first error
hi-pdf-parser batch --from-file files.txt --abort-on-error --out ./out
```

## Gotchas

- hi-pdf-parser only supports **PDF** for `parse`/`batch`. A non-PDF input → exit 10 `INPUT_FORMAT_UNSUPPORTED`; it does no OCR/Office/image conversion, so use a tool that supports those formats instead.
...
- Global flags must precede the subcommand: `hi-pdf-parser -v parse x.pdf`, **not** `hi-pdf-parser parse x.pdf -v` (the latter errors with "unrecognized arguments").
